...
require_once("include/departemen_variables.php");
...
$layout = new TLayout("export2", "Office1City1", "MobileCity1");
$layout->version = 2;
...
$page_layouts["departemen_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_departemen"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["id_departemen"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
