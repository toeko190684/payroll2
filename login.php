...
$layout = new TLayout("login2", "Office1City1", "MobileCity1");
$layout->version = 2;
...
if ((postvalue("state") || postvalue("code")) && $pageObject->LogInFromFacebook()) 
{
	//redirect after success login
	if($myurl) 
	{
		echo"<script>window.opener.location.href = '".$myurl."'; window.close();</script>";
	} 
	else 
	{
		echo"<script>window.opener.location.href = '".GetTableLink("menu")."'; window.close();</script>";
	}
	exit();
}

if (postvalue("error_reason")) 
{
